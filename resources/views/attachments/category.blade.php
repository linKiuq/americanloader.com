...
<head>
    ...
    @include('partials.seo', [
        'title' => $title . ' for Sale | American Loader',
        'description' => $description ?? 'Browse American Loader attachments for wheel loaders, skid steer loaders, STORM mini excavators, and compact construction equipment.',
        'keywords' => $title . ', wheel loader attachments, skid steer attachments, mini excavator attachments, American Loader',
    ])
...
<body class="min-h-screen bg-gray-50 text-[#071d38] flex flex-col">
    @include('partials.header')

    <main class="flex-grow">
        <section class="border-b border-gray-200 bg-white">
            <div class="mx-auto max-w-7xl px-4 py-5 text-sm text-gray-500 sm:px-6 lg:px-8">
                <a href="{{ route('welcome') }}" class="hover:text-red-700">Home</a>
                <span class="mx-2">/</span>
                <a href="{{ route('equipment') }}" class="hover:text-red-700">Equipment</a>
                <span class="mx-2">/</span>
        ...
        <section class="border-b border-gray-200 bg-gradient-to-r from-slate-950 to-slate-800 text-white">
            <div class="mx-auto grid max-w-7xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1fr_auto] lg:items-center lg:px-8">
                <div>
                    <p class="mb-3 text-xs font-black uppercase tracking-[0.3em] text-red-300">Attachment Shop</p>
        ...
        <section class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
            @if (session('success'))
                <div class="mb-7 flex items-center gap-3 rounded-xl border border-green-200 bg-green-50 px-5 py-4 text-sm font-semibold text-green-800">
                    ...
                    <a href="{{ route('cart') }}" class="ml-auto whitespace-nowrap font-black text-red-800">View Cart</a>
                </div>
            @endif
                ...
                <aside class="space-y-6">
                    <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                        ...
                        <nav class="space-y-2 text-sm">
                            <a href="{{ route('attachments.index') }}" class="flex items-center justify-between rounded-xl px-3 py-3 {{ request()->routeIs('attachments.index') ? 'bg-red-50 font-bold text-red-800' : 'text-gray-600 hover:bg-gray-50 hover:text-red-800' }}">
                                <span>All Attachments</span>
                                <span>{{ $attachmentCounts->sum() }}</span>
                            </a>
                            <a href="{{ route('attachments.mini-excavator') }}" class="flex items-center justify-between rounded-xl px-3 py-3 {{ request()->routeIs('attachments.mini-excavator*') ? 'bg-red-50 font-bold text-red-800' : 'text-gray-600 hover:bg-gray-50 hover:text-red-800' }}">
                    ...
                    <div class="rounded-2xl bg-red-700 p-6 text-white">
                        <p class="text-xs font-bold uppercase tracking-[0.24em] text-red-100">Need Compatibility Help?</p>
                        <h2 class="mt-3 text-xl font-black">Find the right fit</h2>
                        <p class="mt-3 text-sm leading-6 text-red-100">Our equipment team can confirm couplers, hydraulic flow, and machine compatibility.</p>
                        <a href="{{ route('contact') }}" class="mt-5 inline-flex rounded-lg bg-white px-5 py-3 text-xs font-black uppercase tracking-wider text-red-800">Contact Sales</a>
                    </div>
                </aside>
                ...
                <section>
                    <form method="GET" action="{{ url()->current() }}" class="mb-7 flex flex-col gap-4 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div class="flex flex-col gap-3 sm:flex-row">
                            <label class="relative">
                                ...
                                <input name="search" value="{{ $search }}" type="search" placeholder="Search attachments" class="w-full rounded-lg border border-gray-300 py-2.5 pl-9 pr-3 text-sm focus:border-red-500 focus:outline-none sm:w-52">
                            </label>
                            <select name="sort" onchange="this.form.submit()" class="rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm focus:border-red-500 focus:outline-none">
                            ...
                            <button type="submit" class="rounded-lg bg-[#071d38] px-4 py-2.5 text-xs font-black uppercase tracking-wider text-white hover:bg-red-800">Filter</button>
                        </div>
                    </form>
                    ...
                    @if ($products->isEmpty())
                        <div class="rounded-2xl border border-dashed border-gray-300 bg-white px-6 py-20 text-center">
                            ...
                            <a href="{{ route('attachments.index') }}" class="mt-6 inline-flex rounded-lg bg-red-700 px-6 py-3 text-sm font-bold text-white">View All Attachments</a>
                        </div>
                    @else
                            ...
                            @foreach ($products as $product)
                                <article class="group flex flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm transition hover:border-red-300 hover:shadow-lg">
                                    <a href="{{ route('product.show', $product['slug']) }}" class="relative flex h-56 items-center justify-center overflow-hidden bg-gray-50 p-5">
                                        <img src="{{ $product['image'] ?? '' }}" alt="{{ $product['name'] }} by TYPHON - {{ $product['category'] ?? 'Attachment' }} for Sale at American Loader" title="{{ $product['name'] }} by TYPHON" loading="lazy" class="max-h-full max-w-full object-contain transition duration-300 group-hover:scale-105">
                                    ...
                                    <div class="flex flex-grow flex-col p-5">
                                        <div class="mb-2 text-xs text-red-500" aria-label="Rated 5 out of 5">
                                            <i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i>
                                        </div>
                                        <a href="{{ route('product.show', $product['slug']) }}" class="line-clamp-2 min-h-[3rem] text-sm font-bold leading-6 transition hover:text-red-800">{{ $product['name'] }}</a>
                                        ...
                                        <div class="mt-auto flex gap-2 pt-5">
                                            <form method="POST" action="{{ route('cart.items.store') }}" class="flex-1">
                                                ...
                                                <button type="submit" class="w-full rounded-lg bg-red-700 px-3 py-3 text-xs font-black uppercase tracking-wider text-white transition hover:bg-red-800">Add to Cart</button>
                                            </form>
                                            <a href="{{ $product['checkoutUrl'] ?? route('store') }}" class="flex-1 rounded-lg border border-gray-300 px-3 py-3 text-center text-xs font-black uppercase tracking-wider text-gray-700 transition hover:border-red-700 hover:text-red-800">Buy Now</a>
                                        </div>
                                    </div>
                                </article>
                            @endforeach
                        ...
                        @if ($products->hasPages())
                            <nav class="mt-10 flex items-center justify-between border-t border-gray-200 pt-7" aria-label="Pagination">
                                ...
                                @else
                                    <a href="{{ $products->previousPageUrl() }}" class="rounded-lg border border-gray-300 bg-white px-5 py-3 text-sm font-bold hover:border-red-500 hover:text-red-800">Previous</a>
                                @endif
                                <span class="text-sm font-semibold text-gray-600">Page {{ $products->currentPage() }} of {{ $products->lastPage() }}</span>
                                @if ($products->hasMorePages())
                                    <a href="{{ $products->nextPageUrl() }}" class="rounded-lg border border-gray-300 bg-white px-5 py-3 text-sm font-bold hover:border-red-500 hover:text-red-800">Next</a>
                                @else
