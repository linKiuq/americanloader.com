...
<head>
    ...
    <title>{{ $title }} - Skoop Loaders</title>
    ...
    <main class="flex-grow">
        <section class="border-b border-gray-200 bg-white">
            <div class="mx-auto max-w-7xl px-4 py-5 text-sm text-gray-500 sm:px-6 lg:px-8">
                ...
                <a href="{{ route('equipment') }}" class="hover:text-blue-600">Equipment</a>
                <span class="mx-2">/</span>
        ...
        <section class="border-b border-gray-200 bg-gradient-to-r from-slate-950 to-slate-800 text-white">
            <div class="mx-auto grid max-w-7xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1fr_auto] lg:items-center lg:px-8">
                <div>
                    <p class="mb-3 text-xs font-black uppercase tracking-[0.3em] text-blue-300">Attachment Shop</p>
        ...
        <section class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
            @if (session('success'))
                <div class="mb-7 flex items-center gap-3 rounded-xl border border-green-200 bg-green-50 px-5 py-4 text-sm font-semibold text-green-800">
                    ...
                    <a href="{{ route('cart') }}" class="ml-auto whitespace-nowrap font-black text-blue-700">View Cart</a>
                </div>
            @endif
                ...
                <aside class="space-y-6">
                    <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                        ...
                        <nav class="space-y-2 text-sm">
                            <a href="{{ route('attachments.index') }}" class="flex items-center justify-between rounded-xl px-3 py-3 {{ request()->routeIs('attachments.index') ? 'bg-blue-50 font-bold text-blue-700' : 'text-gray-600 hover:bg-gray-50 hover:text-blue-700' }}">
                                <span>All Attachments</span>
                                <span>{{ $attachmentCounts->sum() }}</span>
                            </a>
                            <a href="{{ route('attachments.mini-excavator') }}" class="flex items-center justify-between rounded-xl px-3 py-3 {{ request()->routeIs('attachments.mini-excavator*') ? 'bg-blue-50 font-bold text-blue-700' : 'text-gray-600 hover:bg-gray-50 hover:text-blue-700' }}">
                                <span>Mini Excavator</span>
                                <span>{{ $attachmentCounts->get('Mini Excavator Attachments', 0) }}</span>
                            </a>
                            <a href="{{ route('attachments.skid-steer') }}" class="flex items-center justify-between rounded-xl px-3 py-3 {{ request()->routeIs('attachments.skid-steer*') ? 'bg-blue-50 font-bold text-blue-700' : 'text-gray-600 hover:bg-gray-50 hover:text-blue-700' }}">
                    ...
                    <div class="rounded-2xl bg-blue-600 p-6 text-white">
                        <p class="text-xs font-bold uppercase tracking-[0.24em] text-blue-100">Need Compatibility Help?</p>
                        <h2 class="mt-3 text-xl font-black">Find the right fit</h2>
                        <p class="mt-3 text-sm leading-6 text-blue-100">Our equipment team can confirm couplers, hydraulic flow, and machine compatibility.</p>
                        <a href="{{ route('contact') }}" class="mt-5 inline-flex rounded-lg bg-white px-5 py-3 text-xs font-black uppercase tracking-wider text-blue-700">Contact Sales</a>
                    </div>
                </aside>
                ...
                <section>
                    <form method="GET" action="{{ url()->current() }}" class="mb-7 flex flex-col gap-4 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div class="flex flex-col gap-3 sm:flex-row">
                            <label class="relative">
                                ...
                                <input name="search" value="{{ $search }}" type="search" placeholder="Search attachments" class="w-full rounded-lg border border-gray-300 py-2.5 pl-9 pr-3 text-sm focus:border-blue-500 focus:outline-none sm:w-52">
                            </label>
                            <select name="sort" onchange="this.form.submit()" class="rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm focus:border-blue-500 focus:outline-none">
                            ...
                            <button type="submit" class="rounded-lg bg-gray-950 px-4 py-2.5 text-xs font-black uppercase tracking-wider text-white hover:bg-blue-700">Filter</button>
                        </div>
                    </form>
                    ...
                    @if ($products->isEmpty())
                        <div class="rounded-2xl border border-dashed border-gray-300 bg-white px-6 py-20 text-center">
                            ...
                            <a href="{{ route('attachments.index') }}" class="mt-6 inline-flex rounded-lg bg-blue-600 px-6 py-3 text-sm font-bold text-white">View All Attachments</a>
                        </div>
                    @else
                        <div class="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
                            @foreach ($products as $product)
                                <article class="group flex flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm transition hover:border-blue-300 hover:shadow-lg">
                                    <a href="{{ route('product.show', $product['slug']) }}" class="relative flex h-56 items-center justify-center overflow-hidden bg-gray-50 p-5">
                                        ...
                                        <span class="absolute left-4 top-4 rounded-full bg-white px-3 py-1 text-[10px] font-black uppercase tracking-wider text-blue-700 shadow-sm">{{ str_replace(' Attachments', '', $product['category']) }}</span>
                                    </a>
                                    <div class="flex flex-grow flex-col p-5">
                                        <div class="mb-2 text-xs text-amber-500" aria-label="Rated 5 out of 5">
                                            <i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i>
                                        </div>
                                        <a href="{{ route('product.show', $product['slug']) }}" class="line-clamp-2 min-h-[3rem] text-sm font-bold leading-6 transition hover:text-blue-700">{{ $product['name'] }}</a>
                                        ...
                                        <div class="mt-auto flex gap-2 pt-5">
                                            <form method="POST" action="{{ route('cart.items.store') }}" class="flex-1">
                                                ...
                                                <button type="submit" class="w-full rounded-lg bg-blue-600 px-3 py-3 text-xs font-black uppercase tracking-wider text-white transition hover:bg-blue-700">Add to Cart</button>
                                            </form>
                                            <a href="{{ $product['checkoutUrl'] ?? route('store') }}" class="flex-1 rounded-lg border border-gray-300 px-3 py-3 text-center text-xs font-black uppercase tracking-wider text-gray-700 transition hover:border-blue-600 hover:text-blue-700">Buy Now</a>
                                        </div>
                                    </div>
                                </article>
                            @endforeach
                        ...
                        @if ($products->hasPages())
                            <nav class="mt-10 flex items-center justify-between border-t border-gray-200 pt-7" aria-label="Pagination">
                                ...
                                @else
                                    <a href="{{ $products->previousPageUrl() }}" class="rounded-lg border border-gray-300 bg-white px-5 py-3 text-sm font-bold hover:border-blue-500 hover:text-blue-700">Previous</a>
                                @endif
                                <span class="text-sm font-semibold text-gray-600">Page {{ $products->currentPage() }} of {{ $products->lastPage() }}</span>
                                @if ($products->hasMorePages())
                                    <a href="{{ $products->nextPageUrl() }}" class="rounded-lg border border-gray-300 bg-white px-5 py-3 text-sm font-bold hover:border-blue-500 hover:text-blue-700">Next</a>
                                @else
