...
@section('content')
    <div class="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
            <p class="text-xs font-black uppercase tracking-[0.3em] text-red-700">Content Management</p>
        ...
        <a href="{{ route('admin.blog.create') }}" class="rounded-lg bg-red-500 px-6 py-4 text-center text-sm font-black uppercase tracking-wider text-white hover:bg-red-500">New Post</a>
    </div>

    <div class="grid gap-5 md:grid-cols-2 lg:grid-cols-5">
        <a href="{{ route('admin.blog.index') }}" class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm hover:border-red-300">
            <p class="text-xs font-black uppercase tracking-[0.24em] text-slate-500">All Posts</p>
            <p class="mt-4 text-3xl font-black text-[#071d38]">{{ $totalPosts }}</p>
        </a>
        <a href="{{ route('admin.blog.index') }}" class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm hover:border-red-300">
        ...
        <a href="{{ route('admin.categories.index') }}" class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm hover:border-red-300">
            <p class="text-xs font-black uppercase tracking-[0.24em] text-slate-500">Categories</p>
            <p class="mt-4 text-3xl font-black text-[#071d38]">{{ $categoryCount }}</p>
        </a>
        <a href="{{ route('admin.tags.index') }}" class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm hover:border-red-300">
            <p class="text-xs font-black uppercase tracking-[0.24em] text-slate-500">Tags</p>
            <p class="mt-4 text-3xl font-black text-[#071d38]">{{ $tagCount }}</p>
        </a>
    </div>
    ...
    <section class="mt-10 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="flex flex-col justify-between gap-3 border-b border-slate-100 px-6 py-5 sm:flex-row sm:items-center">
            ...
            <a href="{{ route('admin.blog.index') }}" class="text-sm font-bold text-red-800 hover:text-red-700">Manage all posts</a>
        </div>
        <div class="divide-y divide-slate-100">
            @forelse ($recentPosts as $post)
                <div class="flex flex-col gap-4 px-6 py-5 md:flex-row md:items-center md:justify-between">
                    <div>
                        <p class="font-bold text-[#071d38]">{{ $post->title }}</p>
                    ...
                    <div class="flex items-center gap-4 text-sm">
                        ...
                        <a href="{{ route('admin.blog.edit', $post) }}" class="font-bold text-red-800 hover:text-red-700">Edit</a>
                    </div>
                </div>
            @empty
